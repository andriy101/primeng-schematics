...
function addComponentModulesToModule(options) {
    return (host) => {
        ...
        if (modulePath) {
            schematics_2.addModuleImportToModule(host, modulePath, 'ChartModule', 'primeng/chart');
            if (options.type === 'line') {
                schematics_2.addModuleImportToModule(host, modulePath, 'ToastModule', 'primeng/toast');
            }
        }
        return host;
    };
